...
```powershell
Start-Process '.\Agent Burn_0.1.58_x64-setup.exe' -ArgumentList '/UPDATE' -Wait
```

Update mode replaces the application in place without showing the uninstall/data-removal flow. Dashboard settings and history under `%LOCALAPPDATA%\Agent Burn` are preserved. Automatic signed update checks are enabled by default and run at most once per day. An update is never installed without pressing Install update and confirming. The release feed is configured in `tauri.conf.json`.
