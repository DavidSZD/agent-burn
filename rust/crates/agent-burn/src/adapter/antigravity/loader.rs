...
mod tests {
    ...
    use super::super::parser::test_support::{
    ...
    fn snapshots_production_reports_for_focused_periods() {
        ...
        let reports = periods
            .into_iter()
            .map(|kind| {
                let rows = super::super::report::summarize_entries(&entries, kind).unwrap();
                (
                    format!("{kind:?}").to_lowercase(),
                    json!({
                    "json": super::super::report::report_from_rows(&rows, kind),
                    "table": rows.iter().map(|row| json!({
                        "firstColumn": crate::adapter::opencode::first_column(kind),
                        "period": crate::adapter::opencode::summary_period(row),
    ...
    fn propagates_database_open_and_query_errors() {
        let fixture = Fixture::new();
        let _not_a_database = fixture.write_file("conversations/not-a-db.db", "not sqlite");
        let _guard = EnvVarsGuard::set_many([(
            super::super::paths::ANTIGRAVITY_DATA_DIR_ENV,
            Some(OsString::from(fixture.root())),
        )]);
        ...
        assert!(message.contains("not-a-db.db"));
